...
if useHomogeneousFiels == True:
 # trace particles in homogeneous field with out inner or outer boundaries
 Rp = 1e3 # length scale in plots, radius of planet and inner boundary
 Rp_str = 'km' 
 UeHomog = np.array((-400e3,0,0))
 BHomog = np.array((0,1e-9,0))
else:
 # trace test particles in the test run
 #folderRun = os.path.join(os.getenv('HOME'),'bin/corsair/testrun/') # run folder
 folderRun = './' # run folder
 Rp = RMa # length scale in plots, radius of planet and inner boundary
 Rp_str = 'Rp'
 Rp2 = Rp**2
 # use Nreader'th found VLSV file in the run folder
 Nreader = 10
 # set simulation run
...
# initial test particle positions [m]
r0 = np.array([
(2.5*Rp,0,1.5*Rp),
(2.5*Rp,0,1.2*Rp),
(2.5*Rp,0,-1.5*Rp),
...
# initial test particle velocities [m/s]
Usw = (-400e3,0,0) # bulk velocity
vth = 30e3 # random thermal velocity
v0 = np.ones(r0.shape)*Usw + vth*np.random.rand(r0.shape[0],r0.shape[1])

# set up homogeneous field arrays
if useHomogeneousFiels == True:

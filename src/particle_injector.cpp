...
bool InjectorFlow::initialize(Simulation& sim,SimulationClasses& simClasses,ConfigReader& cr,const std::string& configRegionName,const ParticleListBase* plist) {
   ...
   // check injection boundaries string and prepare injection bit mask
	...
	injWalls.erase(new_end,injWalls.end());
	string stmp;
	// split string into substrings separated by whitespaces
	stringstream sstmp(injWalls);
	vector<string> svtmp;
	while (getline(sstmp,stmp,' ')) { svtmp.push_back(stmp); }
